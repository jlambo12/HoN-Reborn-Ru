...
> Версия `v0.1.0-beta.3` исправляет загрузку шрифтов при запуске игры. Автоматические проверки проходят; часть экранов после авторизации и матчей ещё требует ручного визуального QA. Подробности — в [заметках к релизу](docs/release-notes/v0.1.0-beta.3.md).
...
4. Нажмите «Установить перевод», затем запускайте игру официальным ярлыком, через GearUP или напрямую.
...
```powershell
py -3.14 tools/verify_release.py --version 0.1.0-beta.3
.\scripts\package_release.ps1 -Version 0.1.0-beta.3 -PythonCommand "py"
```
