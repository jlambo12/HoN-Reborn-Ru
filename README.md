...
> Версия `v0.1.0-beta.9` исправляет запуск локализации без сброса пользовательских настроек и ошибку отрисовки Launcher после сворачивания окна. Подробности — в [заметках к релизу](docs/release-notes/v0.1.0-beta.9.md).
...
```powershell
py -3.14 tools/verify_release.py --version 0.1.0-beta.9
.\scripts\package_release.ps1 -Version 0.1.0-beta.9 -PythonCommand "py"
```
...
- при установке и обновлении Launcher сохраняет самые свежие настройки между обычным и русским профилями; из ошибочно созданного beta.9 профиля переносится только более свежий `login.cfg`, но не сброшенные игровые настройки; заменяемые файлы предварительно резервируются и не выводятся в лог;
